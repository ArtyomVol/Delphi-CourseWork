...
begin
  ...
  if HLib = 0 then
  begin
    MessageBox(0,'Библиотека ComponentsInitializing.dll не найдена', 'Ошибка',
     mb_Ok);
    Form2.CloseModal;
    Exit;
  end;
  ...
  begin
    MessageBox(0, 'Метод InitializeStringGrid в библиотеке ' +
    'ComponentsInitializing.dll не найден', 'Ошибка', mb_Ok);
    Form2.Close;
  end;
  @AInitializeTabSheet := GetProcAddress(HLib, 'InitializeTabSheet');
  if not Assigned(AInitializeTabSheet) then
  begin
    MessageBox(0, 'Метод InitializeTabSheet в библиотеке ' +
    'ComponentsInitializing.dll не найден', 'Ошибка', mb_Ok);
    Form2.Close;
  end;
end;
